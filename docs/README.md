...
- **[Developer guide](developer-guide.md)** — architecture, the module map, the key
  pipelines, and recipes for extending Biggy.

### Reference
- **[Schema JSON format](schema-json-format.md)** — author a whole app as a single
  import file (for humans and LLMs), with a complete template
  ([`schema-reference.example.json`](schema-reference.example.json)).

The user, designer, setup, and developer guides are also available **inside the
app**: click **Help** in the top bar.
